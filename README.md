...
also if asked about continuing an install at anytime and this is your first install, press `2`; if you're having installing the script after cloinging this repo, be sure that there's no win95 dir inside `/usr/share`; if there is remove it with rm -r example `rm -r /usr/share/win95` and try again; this will restart the download process on everything.
